...
class prop1(Scene):
    def construct(self):
        ...
        #GEOMETRY:
        line = Line(color=RED).shift(RIGHT*4)
        ...
        invisible_height = Line().rotate(PI/2).move_to(UP * length / 2 + DOWN * 0.11 + 2*RIGHT).set_length(length * sqrt(3) / 2).set_opacity(0)
        hyp1 =always_redraw(lambda: Line(color=RED, start=line.get_left(), end=invisible_height.get_top()).shift(2*RIGHT))
        hyp2 =Line(color=RED, start=[1, 0, 0], end=invisible_height.get_top()).shift(2*RIGHT)

        

        #LABELS:
        interlabel = Tex("C").scale(0.5).shift(4*RIGHT + 2.1*UP)
        ...
        self.wait()
        # self.add(circ1, circ2, line, hyp1, hyp2, circlabels, lrlabels, interlabel)